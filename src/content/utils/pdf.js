...
// "Instructor: John Smith" lines, which is all the syllabus parser needs.
...
export async function extractPdfText(input) {
  ...
  while ((lm = litRe.exec(combined))) {
    const text = pdfUnescape(lm[0].slice(1, -1)).trim();
  ...
  while ((hm = hexRe.exec(combined))) {
    ...
    if (clean) literals.push(clean);
    if (literals.length > 20000) break;
  }

  return joinContinuationLines(chunkText(literals.join(' '))).join('\n');
}